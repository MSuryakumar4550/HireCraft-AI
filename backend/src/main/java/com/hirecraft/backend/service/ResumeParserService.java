...
public class ResumeParserService {
    ...
    private final String PYTHON_SERVICE_URL = "http://localhost:8000/api/parse-resume";
    ...
    public ParsedResumeDto parseResume(String rawText) {
        ...
        ByteArrayResource resource = new ByteArrayResource(rawText.getBytes()) {
            ...
            public String getFilename() {
                return "resume.txt"; // Mock extension to pass FastAPI validation
            }
        };
        ...
        try {
            ResponseEntity<ParsedResumeDto> response = restTemplate.postForEntity(
                    PYTHON_SERVICE_URL, requestEntity, ParsedResumeDto.class);
            return response.getBody();
        } catch (Exception e) {
            System.err.println("Error calling Python Resume Parser: " + e.getMessage());
            e.printStackTrace();
            ParsedResumeDto fallback = new ParsedResumeDto();
